...
function getUserPhotosHeader($uid){	
	...
	$photo = "";
	$i=0;
	$photos = $mysqli->query("SELECT thumb,photo FROM users_photos WHERE approved = 1 and blocked = 0 and u_id = '".$uid."' order by profile desc, id desc LIMIT 200");
	if ($photos->num_rows > 0) { 
		while($up = $photos->fetch_object()){
			$photo .= '<div class="relative profile-photo" data-url="photos" data-index="'.$i.'" data-src="'.$up->thumb.'" style="display:inline-block;width:140px;height:140px;cursor:pointer"></div>';
			$i++;
		}	
	}
	return $photo;
